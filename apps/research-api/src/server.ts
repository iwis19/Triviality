...
app.post("/research/jobs", async (request: FastifyRequest<{ Body: CreateJobBody }>, reply) => {
  ...
  await collections.researchEpisodes.insertOne({ _id: episodeId, projectId, title, objective: statement, status: "ACTIVE", area, orchestrator: "workswarm", roleModels, mode, budget, leanStatement: body.leanStatement?.trim() || undefined, atlasProblemId, stage: "Queued for research worker", progress: 2, createdAt: now, updatedAt: now });
