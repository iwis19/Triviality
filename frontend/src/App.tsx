...
export default function App() {
  ...
  useEffect(() => setReducedMotion(systemReduced), [systemReduced]);
  ...
  useEffect(() => {
    ...
    publicApi.problem(problemSlug).then(setDetail).catch((e: Error) => setError(e.message));
  ...
  const exportJson = () => {
    ...
    a.href = url;
    a.download = `mathlab-${problemSlug || areaFilter || "atlas"}.json`;
    a.click();
  ...
  return (
    <div className="app">
      <header>
        ...
        <button className="primary" onClick={() => setPrivateOpen((o) => !o)}>
          {privateOpen ? "Close research controls" : "Research controls"}
        </button>
      </header>
      {error && (
      ...
      <div className="body">
        ...
        <aside className="right">
          {privateOpen ? (
            <PrivatePanel problems={problems} selectedIdeaId={selected?.type === "idea" ? selected.id : null} onChanged={refresh} />
          ) : (
